...
async function execute(command: Command, ws: WebSocket): Promise<Record<string, unknown>> {
  ...
  switch (command.type) {
    ...
    case 'git.branch.create': { const branch = String(payload.branch || ''); if (!/^orlynx-e2e\/[a-zA-Z0-9._-]+$/.test(branch)) throw new Error('Only an isolated orlynx-e2e branch may be created through this operation.'); git(['checkout', '-b', branch]); return { branch }; }
