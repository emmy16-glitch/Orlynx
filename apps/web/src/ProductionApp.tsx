...
type Page = 'welcome' | 'home' | 'projects' | 'github' | 'agents' | 'settings' | 'cloud' | 'tasks' | 'search' | 'workspace' | 'setup';
      ...
      <div className="app-main">
        {page === 'workspace' && session ? <>
          ...
          <div className="workspace-layout">
            <main className="workspace-main">
              {tab === 'chat' && <section className="conversation">
                {!messages.length && <div className="conversation-intro"><span className="agent-avatar"><Icon name="agents" /></span><div><h2>{ai.state === 'ready' || ai.state === 'working' ? 'Work with Orlynx AI' : 'Connect AI to start'}</h2><p>{ai.state === 'ready' || ai.state === 'working' ? `Tasks run on this imported repository${ai.model ? ` with ${ai.model.displayName}` : ''}.` : `Connect an AI account to start working on this project. Your chat stays here.`}</p>{(ai.state === 'disconnected' || ai.state === 'needs_attention' || ai.state === 'error') && <Button onClick={() => setShowConnectAI(true)}><Icon name="agents" />{ai.state === 'error' ? 'AI unavailable — details' : 'Connect AI'}</Button>}</div></div>}
          ...
          <main className="page-body">
            ...
            {!restoring && page === 'settings' && <SettingsScreen integration={integration} theme={theme} setTheme={setTheme} reload={refreshIntegrations} ai={ai} providers={aiProviders} onManageAi={() => setShowConnectAI(true)} />}
