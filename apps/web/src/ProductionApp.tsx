          ...
          <div className="workspace-layout">
            <main className="workspace-main">
              {tab === 'chat' && <section className="conversation">
                {workspacePreparing && <><CloudTransition state={session.workspace.state === 'creating' || session.workspace.state === 'starting' || session.workspace.state === 'bootstrapping' ? 'preparing' : 'connecting'} /><p className="workspace-timer" role="status">{workspaceStalled ? 'This step has stalled' : 'Time in this step'} · {Math.floor(workspaceSeconds / 60)}:{String(workspaceSeconds % 60).padStart(2, '0')}{!workspaceStalled && ' · We will flag it if it takes over 3 minutes.'}</p></>}
                ...
                {!messages.length && <div className="conversation-intro setup-aware"><span className="agent-avatar"><span className="brand-mark small-mark" /></span><div>
                  ...
                  <h2>{ai.state === 'ready' || ai.state === 'working' ? 'What should we work on?' : !aiAccountConnected ? 'Connect AI' : workspaceStalled ? 'Workspace is taking too long' : workspacePreparing ? 'Preparing your workspace…' : workspaceDisconnected ? 'Reconnect your workspace' : workspaceReady ? 'Choose a model to start' : 'Start your workspace'}</h2>
                  <p>{ai.state === 'ready' || ai.state === 'working'
                    ...
                    : !aiAccountConnected
                      ...
                      : workspaceStalled
                        ? 'This step has passed the three-minute wait limit. Retry the connection without creating a new conversation.'
                      : workspacePreparing
                        ? session.workspace?.state === 'creating' || session.workspace?.state === 'starting' ? 'GitHub is creating or starting your Codespace.' : session.workspace?.state === 'bootstrapping' ? 'Orlynx is installing and starting the agent inside your Codespace.' : 'The agent is connecting and checking OpenCode.'
                  ...
                  <div className="setup-actions">
                    ...
                    {aiAccountConnected && !workspaceReady && (!workspacePreparing || workspaceStalled) && integration.workspace?.cloudAvailable && <Button onClick={() => startCloud(workspaceDisconnected || workspaceStalled)} disabled={cloudBusy}><Icon name="cloud" />{cloudBusy ? 'Preparing workspace…' : workspaceDisconnected || workspaceStalled ? 'Reconnect workspace' : 'Start workspace'}</Button>}
                ...
                {lastRun?.state === 'queued' && <p className="run-receipt">Queued · Orlynx will start this automatically.</p>}
