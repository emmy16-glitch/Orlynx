  ...
  return (
    <div className={`orlynx-app ${page === 'workspace' ? 'is-workspace' : ''} ${page === 'welcome' ? 'is-welcome' : ''}`}>
      {page !== 'welcome' && onboarded && <aside className="sidebar">
        ...
        <nav className="side-nav" aria-label="Main navigation">{globalNav.map(([id, label, icon]) => <button key={id} className={page === id ? 'selected' : ''} onClick={() => setPage(id)}><Icon name={icon} />{label === 'Agents' ? 'AI' : label}</button>)}</nav>
      ...
      <div className="app-main">
        {page === 'workspace' && session ? <>
          ...
          <main className="page-body">
            ...
            {!restoring && page === 'agents' && <section className="screen-section"><div className="screen-heading"><div><p className="eyebrow">ORLYNX AI</p><h1>AI</h1><p className="screen-subtitle">{ai?.state === 'ready' ? `Ready${ai?.model ? ` · ${ai.model.displayName}` : ''}.` : ai?.state === 'working' ? 'Working on your project.' : 'Connect an AI account to start working.'}</p></div></div><section className="agent-config-row"><span className="agent-avatar"><Icon name="agents" /></span><div><b>Orlynx AI</b><p>{ai?.state === 'ready' || ai?.state === 'working' ? 'Ready to work on your projects.' : 'Not connected yet.'}</p></div><Badge tone={ai?.state === 'ready' || ai?.state === 'working' ? 'ok' : 'fail'}>{ai?.state === 'ready' ? 'Ready' : ai?.state === 'working' ? 'Working' : 'Not connected'}</Badge><Button onClick={() => setShowConnectAI(true)}>{aiProviders.some((p: any) => p.state === 'connected') ? 'Manage AI' : 'Connect AI'}</Button></section><details className="advanced-details"><summary>Advanced</summary><p>Engine status: {integration.agent?.connected ? 'online' : 'offline'}</p>{integration.agent?.agents?.length ? <p>Available assistants: {integration.agent.agents.map((a: any) => a.name).join(', ')}</p> : null}</details>{session && <Button tone="ghost" onClick={() => setPage('workspace')}>Return to project</Button>}</section>}
            {!restoring && page === 'cloud' && <section className="screen-section"><div className="screen-heading"><div><p className="eyebrow">CLOUD</p><h1>Cloud workspace</h1><p className="screen-subtitle">Work on your projects from anywhere.</p></div></div><EmptyState title="Cloud workspace is temporarily unavailable" hint="Please try again shortly." /><div className="action-row"><Button tone="ghost" onClick={refreshIntegrations}>Retry</Button>{session && <Button tone="ghost" onClick={() => setPage('workspace')}>Return to project</Button>}</div></section>}
          ...
          <nav className="mobile-global-nav" aria-label="Main navigation">{globalNav.map(([id, label, icon]) => <button key={id} className={page === id ? 'selected' : ''} onClick={() => setPage(id)}><Icon name={icon} /><span>{label === 'Agents' ? 'AI' : label}</span></button>)}</nav>
        </>}
      </div>
    </div>
  );
