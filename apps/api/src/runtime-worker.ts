...
function bootstrapScript(workspace: WorkspaceRecord, values: Values, bridgeUrl: string): string {
  const bridge = fs.readFileSync(bridgeBundle, 'utf8');
  const env = [`ORLYNX_CONTROL=${bridgeUrl}`, `ORLYNX_WORKSPACE_TOKEN=${values.bridgeToken}`, `ORLYNX_WORKSPACE_ID=${workspace.id}`, `ORLYNX_SESSION_ID=${workspace.sessionId}`, `ORLYNX_USER_ID=${workspace.userId}`, `ORLYNX_CONNECTION_ID=${values.connectionId}`, `OPENCODE_SERVER_PASSWORD=${values.openCodePassword}`].map((line) => encoded(line)).join(' ');
...
async function bootstrapWithSandbox(workspace: WorkspaceRecord, values: Values, githubUserToken: string, bridgeUrl: string): Promise<void> {
  ...
  try {
    ...
    await sandbox.writeFiles([{ path: '/tmp/orlynx-bootstrap.sh', content: bootstrapScript(workspace, values, bridgeUrl), mode: 0o600 }]);
...
export async function bootstrapWorkspace(workspace: WorkspaceRecord, values: Values): Promise<void> {
  ...
  if (!base && (process.env.VERCEL === '1' || process.env.ORLYNX_BOOTSTRAP_MODE === 'sandbox')) return bootstrapWithSandbox(workspace, values, githubUserToken, bridgeUrl);
