...
const agentName = process.env.OPENCODE_AGENT || '';
const defaultModel = process.env.OPENCODE_MODEL || '';
const requestTimeout = Number(process.env.OPENCODE_REQUEST_TIMEOUT_MS || 15_000);
const projectsRoot = path.resolve(process.env.OPENCODE_PROJECTS_ROOT || path.join(dataDir, 'repos'));
